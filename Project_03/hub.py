...
# NB: Make sure to add flask_user UserMixin as this adds additional fields and
# properties required by Flask-User
...
class Channel(db.Model):
    ...
    endpoint = db.Column(db.String(100, collation='NOCASE'),
                         nullable=False,
                         unique=True)
...
def create_channel():
    ...
    if request.headers['Authorization'] != 'authkey ' + SERVER_AUTHKEY:
        return "Invalid authorization header ({})".format(
            request.headers['Authorization']), 400
...
def get_channels():
    ...
    return jsonify(channels=[{'name': c.name,
                              'endpoint': c.endpoint,
                              'authkey': c.authkey} for c in channels]), 200
